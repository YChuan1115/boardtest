...
int test_func_uart       (board_device * self)
{
	...
	for(ii = 0 ;ii<30 ;ii++ ) test_buf[ii] = '\0';//\0
	...
	for(ii = 0 ;ii<30 ;ii++ ) test_buf[ii] = 'n';
    printf("test buf not init: %s\n", test_buf);
	// TEST ttyAMA0 Send Message to PL2303
	printf("PL2303 send: %s\n",test_msg);
	int n;
	n=write(usb_tty.fd, test_msg,strlen(test_msg) );
	printf("write %d\n",n);
	usleep(500000/*100000*/);
	//n=read(ama_tty.fd, test_buf,30);
	n=nonblock_read_tty(&ama_tty,test_buf,30);
	printf("read %d\n",n);
	//perror("read:");
